        ...
        <div style={{ padding: '20px', backgroundColor: 'var(--bg-card)', borderRadius: '8px', border: '1px solid var(--border-color)', boxShadow: '0 4px 6px rgba(0,0,0,0.05)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '15px' }}>
                ...
                <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap' }}>
                    {/* YENİ: DIŞA AKTAR BUTONLARI (Sadece rapor varsa görünür) */}
                    {reportText && !isLoading && (
                        <>
                            <button
                                onClick={() => exportToPDF(reportElementId, 'Anlik_Analiz_Raporu.pdf')}
                                style={{ padding: '8px 12px', backgroundColor: 'transparent', color: 'var(--color-error)', border: '1px solid var(--color-error)', borderRadius: '4px', cursor: 'pointer', fontWeight: 'bold', display: 'flex', alignItems: 'center', gap: '5px' }}>
                                📄 PDF
                            </button>
                            <button
                                onClick={() => exportToHTML(reportElementId, 'Anlik_Analiz_Raporu.html')}
                                style={{ padding: '8px 12px', backgroundColor: 'transparent', color: 'var(--color-info)', border: '1px solid var(--color-info)', borderRadius: '4px', cursor: 'pointer', fontWeight: 'bold', display: 'flex', alignItems: 'center', gap: '5px' }}>
                                🌐 HTML
                            </button>
                        </>
                    )}
            ...
            {reportText && (
                <div
                    id={reportElementId} // PDF için ID eklendi
                    style={{
                        marginTop: '25px',
                        padding: '25px',
